...
public class SMTPStateMachine {
	...
	//the quit command makes the state maxhine adopt its initial state because the previous SMTP connection is done
	if(token instanceof QuitCommand){
	    lastSeenTokenType = NO_COMMAND;
	    return true;
	    //no break is used because the return ends the method
	}
	 ...
	 * state machine is one of the four token types above.
	 */
	return false;
    }
